...
if (!String.prototype.hasOwnProperty("padRight")) {
  String.prototype.padRight = function (length, character) {
    ...
    var string = this;
    for (var i = this.length; i < length; ++i) string += character;
...
function getElementValue(element) {
  switch (element.tagName) {
    case "SELECT":
      ...
      if (value.indexOf(";") == 1) return value.substr(2);
      return value;
    case "INPUT":
...
RequirementsBase.prototype.addRequirement = function (id, description) {
  if (this.satisfied_.hasOwnProperty(id)) delete this.satisfied_[id];
...
RequirementsBase.prototype.satisfyRequirement = function (id) {
  if (!this.requirements_.hasOwnProperty(id)) return;
...
RequirementsBase.prototype.verifyRequirements = function (silent) {
  ...
  if (!unsatisfiedRequirements.length) return true;

  if (!silent) alert(unsatisfiedRequirements);

  return false;
};

RequirementsBase.prototype.resetRequirements = function () {
  for (var id in this.satisfied_) this.requirements_[id] = this.satisfied_[id];
...
GeneratorBase.prototype.serialize = function (state) {
  ...
  Object.keys(state).forEach(function (name) {
    var value =
      state[name].index !== undefined ? state[name].index : state[name].value;
...
GeneratorBase.prototype.deserialize = function (serialization) {
  if (!serialization.startsWith("#")) return;
  ...
  fields.forEach(function (field) {
    ...
    self.serialized_state_[field.substr(0, valueIndex)] = field.substr(
      valueIndex + 1
    );
  });
};
...
GeneratorBase.prototype.initializeField = function (name) {
  ...
  if (field.element.tagName == "SELECT") {
    field.defaultValue =
      field.element.options[field.element.selectedIndex].getAttribute(
        "data-id"
      );
  ...
  if (this.serialized_state_.hasOwnProperty(name)) {
    ...
    switch (field.element.tagName) {
      case "INPUT":
        ...
        else field.element.value = value;
        break;
      case "SELECT":
...
GeneratorBase.prototype.getField = function (state, name, defaultValue) {
  if (!state.hasOwnProperty(name)) return defaultValue;

  switch (state[name].type) {
    case GeneratorBase.FIELD_TYPE_ARRAY:
      if (!state[name].value.length) return defaultValue;
    ...
    case GeneratorBase.FIELD_TYPE_BUTTONS:
      ...
      for (var index = 0; index < buttons.length; ++index) {
        ...
        if (values.length > 1) action.icon = values[1];
        if (values.length > 2) action.type = values[2];
        if (values.length > 3) action.placeholder = values[3];
        actions.push(action);
      }
...
GeneratorBase.prototype.resolveFieldState = function (name) {
  ...
  switch (field.element.tagName) {
    case "INPUT":
      if (field.element.type == "checkbox") value = field.element.checked;
      else value = field.element.value;
      break;
    case "SELECT":
...
GeneratorBase.prototype.computeState = function (includeDefault) {
  ...
  Object.keys(this.fields_).forEach(function (name) {
    ...
    if (
      ((fieldState.index !== undefined && fieldState.index == defaultValue) ||
